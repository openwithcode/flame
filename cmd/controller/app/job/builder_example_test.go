...
func Test_asyncfl_hier_mnist(t *testing.T) {
	...
	err = dbService.CreateDesignCode(userID, designID, "async_hier_mnist", "zip", designCodeFile)
	assert.NoError(t, err)
	...
	exampleConfigPath := rootExample + "/example_config_files"

	for i, task := range tasks {
		var jobConfigData objects.JobConfig
		readFileToStruct(t, fmt.Sprintf("%s/%d.json", exampleConfigPath, i+1), &jobConfigData)
...
func connect(t *testing.T) *mongo.Client {
	...
	mongoServer := newMongoServer(t)

	opt := options.Client().ApplyURI(mongoServer.URI())
...
func newMongoServer(t *testing.T) *memongo.Server {
